...
abstract class AbstractMyLocation extends DatabaseEntity implements MyLocationInterface, FulltextSearch{
	...
	public function getMatchCount() {
		Utils::assertTrue("Not allowed to invoke method on unloaded entity", !$this->isNew());
		$col = $this->getSpotColumn();
		$pq = new PreparedQuery("reflirt_nieuw");
		$query = "SELECT count(*) AS aantal " .
				" FROM ".$this->getTable()->getDatabaseName().".".
				$this->getTable()->getTablename().
				" WHERE ".$col."='".$this->getString($col)."'".
				" GROUP BY ".$col.
				" ORDER BY aantal ";
		$pq->setQuery($query);
		$rs = $pq->execute();
		Utils::assertTrue("Resultset > 1", count($rs) < 2);
		return $rs[0]["aantal"];
...
final class MyLocationSimpleSearchMatchSearcher extends Searcher {
	...
	public function getFields(DataSource $ds) {
		return "*, MATCH (keywords) against ('".mysql_real_escape_string($ds->getString("keywords"))."' IN BOOLEAN MODE) as relevance";
	}
	...
	public function getSpot() {
    	if ($this->spot == null) {
			...
			$spot = SpotFactory::newInstance($cat);
			$systemUser = UserFactory::getSystemUser();
			$systemid = $this->getString($this->getSpotColumn());
			$this->spot = EntityFactory::loadEntity(new $spot(), $systemUser, $systemid);
    	}
    	return $this->spot;
    }
}
