...
function smarty_block_t($params, $text, &$smarty)
{
	...
	if($text=="") {
		...
		$currentPage = Translator::getCurrentPage();
		$file = PAGES.$currentPage."/".$currentPage."_".$language."_".$country.".props"; //"subscribe_nl_NL.props";
		$text = translate($file, $text);
	}
...
function translate($file, $line) {
	...
	while($dirName!= PAGES) {
		$f->dirUp();
		$dirName = $f->getDirname();
		$file = $dirName."shared_".Translator::getLanguage()."_".Translator::getCountry().".props";
		if(file_exists($file)) {
			break;
		}
		if($dirName == PAGES) {
			if(DEBUG) {
				//TODO: moet gelogd worden
